...
RUN apt update && apt install -y libvulkan-dev glslc cmake git build-essential
...
RUN apt update && apt install -y mesa-vulkan-drivers libgomp1 --no-install-recommends && rm -rf /var/lib/apt/lists/*
